...
function generateTestFile(tag, endpoints, baseUrl, authHeader, fwConfig) {
  ...
  for (const { name, method, path, pathParams, queryParams, bodyFields, responses, operationId } of endpoints) {
    ...
    // Build URL with PLACEHOLDER_ substitutions for path params
    let urlStr = baseUrl.replace(/\/$/, '') + path;
    for (const p of pathParams) {
      urlStr = urlStr.replace(`{${p}}`, `PLACEHOLDER_${p.toUpperCase()}`);
    }
    ...
    lines.push(`    const response = await fetch('${urlStr}', {`);
...
function generateReadme(fwConfig) {
  return [
    ...
    `## Before Running`,
    ``,
    `1. Replace every \`PLACEHOLDER_*\` value in test URLs with real test data.`,
    `2. If an \`--auth-header\` was injected, update the \`AUTH_HEADERS\` constant in each test file.`,
    `3. For endpoints with a request body, add the body to the \`fetch\` call (see the TODO comment).`,
    ``,
  ].join('\n');
}
